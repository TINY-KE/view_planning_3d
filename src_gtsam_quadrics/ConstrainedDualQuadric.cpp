...
ConstrainedDualQuadric ConstrainedDualQuadric::constrain(
  ...
  if (dual_quadric(3, 3) != 1.0) {
    // 如果 dual_quadric(3, 3) 不等于 1，那么就将整个矩阵除以这个值。
    // 这样可以确保齐次坐标系中的最后一个元素为 1，这是标准的椭球体表示形式。
  ...
  // 点二次曲面矩阵（point quadric matrix） 是双二次曲面矩阵的逆矩阵
  ...
  // 5. 提取旋转矩阵：
  // extract rotation
  // 通过对点二次曲面矩阵的前 3x3 块（表示旋转部分）进行特征值分解，提取出椭球体的旋转矩阵。
  Eigen::EigenSolver<Eigen::Matrix<double, 3, 3>> s(
      normalized_point_quadric.block(0, 0, 3, 3));
  // eigenvectors() 函数返回的是特征向量，这些特征向量组成了旋转矩阵。
  ...
  // 7. 返回 ConstrainedDualQuadric 对象：
