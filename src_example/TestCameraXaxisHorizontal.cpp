...
int main(int argc, char** argv){
	...
	// 三、构建因子
	// (1) 构建末端和水平面的夹角因子
	double sigma = 0.05;  // 避障球与平面的最小距离
	CameraXaxisHorizontal<ArmModel>  factor(0,  *arm_model,  sigma);
    ...
    while (ros::ok())
    {
    	...
    	err_planearm = factor.evaluateError(config, &H1_planearm);
    	std::cout<< "[debug] err_planearm: " << err_planearm << std::endl;
    	std:;cout<<std::endl;
